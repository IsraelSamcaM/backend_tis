...
export const getReservas = async (req, res) => {
  try {
    ...
    const arrayIdsAmbientes = mapearAmbientes(
      ambientes,
      cantidadEst,
      tipoAmbiente
    );
    ...
    const disponiblesAmbienteDia = await obtenerDisponibles(
      arrayIdsAmbientes,
      dia,
      periodosArray
    );
    ...
    const resultadoFiltrado = disponiblesAmbienteDia.filter(
      (disponible) => !idsExcluir.includes(disponible.id_disponible)
    );

    const ambientesDisponibles = await obtenerDetallesReservas(
      resultadoFiltrado,
      fechaInicial
    );
...
const obtenerDetallesReservas = async (
  disponiblesAmbienteDia,
  fechaReserva
) => {
